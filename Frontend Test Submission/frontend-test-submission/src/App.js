...
//helping to check url format validity
...
//storing url mapping for testing purpose in local storage
...
//geting the original ul from the shorted url 
...
//this component will show the form and result for the application
function UrlShortenerPage() {
  ...
  //token generated from the given link in the document for authencation purpose
  ...
  const validateInputs = () => {
    const errs = inputs.map((input) => {
      ...
      if (input.validity && (!/^[0-9]+$/.test(input.validity) || parseInt(input.validity) <= 0)) {
        err.validity = 'Validity must be a positive integer in minutes';
      }
      if (input.shortcode && !/^[a-zA-Z0-9_-]{3,}$/.test(input.shortcode)) {
        err.shortcode = 'Shortcode must be at least 3 characters, alphanumeric, - or _ ';
      }
      return err;
    });
    setErrors(errs);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validateInputs()) {
      await Log('frontend', 'warn', 'page', 'Validation failed :', token);
      return;
    }
    setSubmitting(true);
    setResults([]);
    ...
    <Paper sx={{ p: 3 }}>
      ...
      <form onSubmit={handleSubmit}>
        <MuiStack spacing={2}>
          ...
          <Button onClick={addInput} disabled={inputs.length >= mxUrl}>Add URL</Button>
